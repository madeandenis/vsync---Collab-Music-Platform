...
export const CreateGroupForm = ({ setOpen }: CreateGroupFormProps) => {
    ...
    const { state, setters } = useGroupForm();
            ...
            <div className="container mx-auto px-6 py-4 w-3/4 max-w-2xl rounded-xl bg-graphite" onClick={handleContentClick}>
                ...
                <div className="mt-8 flex gap-6 justify-between items-center">
                    ...
                    <div className="relative shadow-hover-black">
                        ...
                        <Thumbnail size={186} src={state.thumbnailSrc} placeHolder={<FaUsers size={80} />} alt='Placeholder for group thumbnail selector' />
                        ...
                        {/* Clear thumbnail selection */}
                        {
                            state.thumbnailSrc && (
                                <button
                                    className="absolute w-1/3 bottom-2 left-1/2 transform -translate-x-1/2 text-black/80 text-2xl font-bold bg-red-500/25 hover:bg-red-500/50 rounded-xl transition-all"
                                    onClick={() => setters.setThumbnailSrc(undefined)}
                                >
                                    &#215;
                                </button>
                            )}
